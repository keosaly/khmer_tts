...
def write_output_sentence(filename, sentence):
    ...
    f.close()
#indir='/home01/SLC/intern/saly.keo/kh_new_seg/word_seg/test/'
#outdir='/home01/SLC/intern/saly.keo/kh_new_seg/data/'
def main():
    text = read_input_sentence('./input.txt')
    res  = segment_text(text)
    tt=res.replace(' ','\n').replace('\n\n\n','')
    write_output_sentence('output.txt', tt)
    print "finish"
